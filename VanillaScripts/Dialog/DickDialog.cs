    ...
    public class DickDialog : Dick, IDialogScript
    {
        public bool CheckPrecondition(GameObject npc, GameObject pc, int lineNumber, out string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 231:
                case 232:
                    originalScript = "game.quests[39].state == qs_mentioned and pc.skill_level_get(npc, skill_bluff) >= 8";
                    return GetQuestState(39) == QuestState.Mentioned && pc.GetSkillLevel(npc, SkillId.bluff) >= 8;
                case 233:
                case 234:
                    originalScript = "game.quests[39].state == qs_completed and pc.skill_level_get(npc, skill_bluff) >= 8";
