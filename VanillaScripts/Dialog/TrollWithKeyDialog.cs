    ...
    public class TrollWithKeyDialog : TrollWithKey, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                case 2:
                case 3:
                    Trace.Assert(originalScript == "pc.skill_level_get(npc,skill_bluff) >= 1 and npc.item_find(4401) != OBJ_HANDLE_NULL and not npc.has_met(pc)");
                ...
                case 41:
                case 42:
                    Trace.Assert(originalScript == "pc.skill_level_get(npc,skill_bluff) >= 1 and not npc.has_met(pc)");
                    return pc.GetSkillLevel(npc, SkillId.bluff) >= 1 && !npc.HasMet(pc);
                case 43:
                case 44:
                    Trace.Assert(originalScript == "pc.skill_level_get(npc,skill_intimidate) >= 8 and npc.item_find(4401) != OBJ_HANDLE_NULL");
