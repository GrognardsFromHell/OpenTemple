    ...
    public class AlrremDialog : Alrrem, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 310:
                case 311:
                    Trace.Assert(originalScript == "game.global_flags[110] == 0 and game.quests[46].state == qs_completed and ( game.quests[52].state == qs_mentioned or game.quests[52].state == qs_accepted )");
                ...
                case 406:
                case 409:
                    Trace.Assert(originalScript == "game.global_flags[119] == 1 and game.quests[44].state >= qs_accepted");
                    return GetGlobalFlag(119) && GetQuestState(44) >= QuestState.Accepted;
                case 407:
                case 408:
                    Trace.Assert(originalScript == "pc.skill_level_get(npc, skill_bluff) >= 11 and npc.has_met( pc )");
        ...
        public void ApplySideEffect(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 530:
                    ...
                    break;
                case 580:
                    Trace.Assert(originalScript == "game.map_flags( 5078, 0, 1 )");
                    // FIXME: map_flags;
                    break;
                case 603:
                case 604:
