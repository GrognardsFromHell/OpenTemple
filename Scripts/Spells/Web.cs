...
namespace Scripts.Spells
{
    ...
    public class Web : BaseSpellScript
    {
        ...
        public override void OnSpellEffect(SpellPacketBody spell)
        {
            ...
            var activeList = Co8PersistentData.getData/*Unknown*/(WEB_KEY);
            if (isNone(activeList))
            {
                activeList = new List<GameObjectBody>();
            }

            activeList.Add(new[] { spell.spellId, derefHandle(web_obj) });
            Co8PersistentData.setData/*Unknown*/(WEB_KEY, activeList);
        }
        // End of Section		#
        ...
        public override void OnEndSpellCast(SpellPacketBody spell)
        {
            ...
            if (isNone(activeList))
            {
                Logger.Info("ERROR! Active Web spell without activeList!");
                return;
            }
            ...
            foreach (var entry in activeList)
            {
                ...
                if (spellID == spell.spellId)
                {
                    ...
                    {
                        Co8PersistentData.removeData/*Unknown*/(WEB_KEY);
                        break;
