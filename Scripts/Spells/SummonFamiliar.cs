...
;
        ...
        public override void OnSpellEffect(SpellPacketBody spell)
        {
            ...
            var inv_proto = FindFamiliarProto(spell.caster, 0);
            ...
            var familiar_proto_id = FindFamiliarProto(spell.caster, 1);
            ...
            ID_number = ID_number ^ RandomRange(1, 2147483647); // xor with next "random" number in line, should be more random
                                                                // create familiar
            ...
            var prev_max_hp = familiar.GetInt(obj_f.item_pad_i_1); // familiar's max hp from last time summoned ( 0 if never summoned before)
            var prev_curr_hp = familiar.GetInt(obj_f.item_pad_i_2); // familiar's current xp from last time stowed ( 0 if never summoed before)
            if ((base_hp <= f_hp)) // if 1/2 master's hp is greater than base hp from proto, will use 1/2 masters hp
            {
                var new_hp = familiar_obj.SetBaseStat(Stat.hp_max, f_hp);
            }

            var curr_max_hp = familiar_obj.GetStat(Stat.hp_max); // familiar's max hp from current summons
            var hp_diff = (curr_max_hp - prev_max_hp); // difference between max hp from last time summoned and max hp now ( 0 if master has not gained a level since)
            if ((prev_max_hp != 0)) // has been summoned before
            {
                if ((hp_diff >= 1)) // adds gained hp if master has gained hp since last time summoned
                {
                    var hp_now = prev_currr_hp + hp_diff;
                }
                else
                {
                    var hp_now = prev_curr_hp;
                }
            ...
            var armor = familiar_obj.SetInt(obj_f.npc_ac_bonus, (f_level)); // Natrual Armor bonus
            if ((master_level >= 11))
            {
                var spell_resistance = familiar_obj.AddCondition("Monster Spell Resistance", (5 + master_level), 0); // spell resistance
            }
            ...
            {
                var fortitude_save = familiar_obj.SetInt(obj_f.npc_save_fortitude_bonus, fortitude_bonus);
            }

            var reflex_bonus = Reflex(spell.caster);
            if ((reflex_bonus >= 3))
            {
                var reflex_save = familiar_obj.SetInt(obj_f.npc_save_reflexes_bonus, reflex_bonus);
            }

            var will_bonus = Will(spell.caster);
            if ((will_bonus >= 1))
            {
                var wlll_save = familiar_obj.SetInt(obj_f.npc_save_willpower_bonus, will_bonus);
            }
            ...
            spell.Targets[0].Object = familiar_obj;
            spell.EndSpell();
        }
        ...
        public static int FindFamiliarProto(GameObjectBody master, int x)
        {
        ...
        public static GameObjectBody GetFamiliarHandle(SpellPacketBody spell, FIXME familiar_proto_id)
        {
        ...
        public static int Fortitude(GameObjectBody npc)
        {
            ...
            var bonus = 0;
            var level = npc.GetStat(Stat.level_barbarian) + npc.GetStat(Stat.level_cleric) + npc.GetStat(Stat.level_druid) + npc.GetStat(Stat.level_fighter) + npc.GetStat(Stat.level_paladin) + npc.GetStat(Stat.level_ranger) + npc.GetStat(Stat.level_monk);
            if ((level != 0))
            {
                bonus = ((level / 2) + 2);
            }

            level = npc.GetStat(Stat.level_bard) + npc.GetStat(Stat.level_rogue) + npc.GetStat(Stat.level_sorcerer) + npc.GetStat(Stat.level_wizard);
            if ((level != 0))
            {
                bonus = bonus + (level / 3);
            }
        ...
        public static int Reflex(GameObjectBody npc)
        {
            ...
            var bonus = 0;
            var level = npc.GetStat(Stat.level_barbarian) + npc.GetStat(Stat.level_cleric) + npc.GetStat(Stat.level_druid) + npc.GetStat(Stat.level_fighter) + npc.GetStat(Stat.level_paladin) + npc.GetStat(Stat.level_sorcerer) + npc.GetStat(Stat.level_wizard);
            if ((level != 0))
            {
                bonus = (level / 3);
            }

            level = npc.GetStat(Stat.level_ranger) + npc.GetStat(Stat.level_rogue) + npc.GetStat(Stat.level_monk) + npc.GetStat(Stat.level_bard);
            if ((level != 0))
            {
        ...
        public static int Will(GameObjectBody npc)
        {
            ...
            var bonus = 0;
            var level = npc.GetStat(Stat.level_bard) + npc.GetStat(Stat.level_cleric) + npc.GetStat(Stat.level_druid) + npc.GetStat(Stat.level_monk) + npc.GetStat(Stat.level_sorcerer) + npc.GetStat(Stat.level_wizard);
            if ((level != 0))
            {
                bonus = ((level / 2) + 2);
            }

            level = npc.GetStat(Stat.level_barbarian) + npc.GetStat(Stat.level_fighter) + npc.GetStat(Stat.level_paladin) + npc.GetStat(Stat.level_ranger) + npc.GetStat(Stat.level_rogue);
            if ((level != 0))
            {
                bonus = bonus + (level / 3);
            }
