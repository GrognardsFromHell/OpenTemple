    ...
    public class JarooDialog : Jaroo, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 8:
                case 9:
                    Trace.Assert(originalScript == "game.story_state >= 2 and game.party_alignment == TRUE_NEUTRAL and game.global_flags[19] == 0 and game.global_vars[562] == 4");
                ...
                case 14:
                case 15:
                    Trace.Assert(originalScript == "pc.item_find( 12900 ) == OBJ_HANDLE_NULL and npc.has_met(pc) and game.quests[99].state == qs_accepted");
                ...
                case 103:
                    Trace.Assert(originalScript == "game.party_alignment == LAWFUL_NEUTRAL or game.party_alignment == CHAOTIC_NEUTRAL or game.party_alignment == TRUE_NEUTRAL");
                ...
                case 701:
                case 702:
                    Trace.Assert(originalScript == "game.global_flags[855] == 0");
                    return !GetGlobalFlag(855);
                case 603:
                case 604:
                case 703:
                case 704:
                    Trace.Assert(originalScript == "game.global_flags[855] == 1");
                    return GetGlobalFlag(855);
                case 641:
                    Trace.Assert(originalScript == "game.global_vars[523] <= 1");
                    return GetGlobalVar(523) <= 1;
                case 642:
                    Trace.Assert(originalScript == "game.global_vars[523] >= 2");
                    return GetGlobalVar(523) >= 2;
                case 643:
                    Trace.Assert(originalScript == "game.global_vars[524] <= 3");
                    return GetGlobalVar(524) <= 3;
                case 644:
                    Trace.Assert(originalScript == "game.global_vars[524] >= 4");
        ...
        public void ApplySideEffect(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 324:
                case 325:
                    Trace.Assert(originalScript == "game.quests[72].state = qs_accepted; game.global_vars[562] = 4; game.worldmap_travel_by_dialog(2)");
                ...
                case 326:
                case 327:
                    Trace.Assert(originalScript == "game.quests[72].state = qs_accepted; game.global_vars[562] = 4");
                    SetQuestState(72, QuestState.Accepted);
                    SetGlobalVar(562, 4);
                    ;
                    break;
                case 336:
                    ...
                    break;
                case 346:
                    Trace.Assert(originalScript == "game.quests[26].state = qs_completed; game.quests[72].state = qs_completed; game.global_vars[562] = 6");
                ...
                case 421:
                case 423:
                    Trace.Assert(originalScript == "pc.money_adj(-21000); npc.cast_spell( spell_neutralize_poison, picker_obj )");
                    pc.AdjustMoney(-21000);
                ...
                case 501:
                case 503:
                    Trace.Assert(originalScript == "pc.money_adj(-25000); npc.cast_spell( spell_neutralize_poison, picker_obj )");
                    pc.AdjustMoney(-25000);
                    npc.CastSpell(WellKnownSpells.NeutralizePoison, PickedObject);
                    ;
                    break;
                ...
                case 652:
                    ...
                    break;
                case 660:
                    Trace.Assert(originalScript == "game.picker( npc, spell_reincarnation, should_resurrect_on, [ 690, 640, 710 ] )");
                    // FIXME: picker;
                    break;
                case 683:
                case 684:
                    Trace.Assert(originalScript == "amii_dies_due_to_dont_give_a_damn_dialog(npc,pc)");
                    amii_dies_due_to_dont_give_a_damn_dialog(npc, pc);
                    break;
                case 710:
                    Trace.Assert(originalScript == "game.global_vars[523] = game.global_vars[523] + 1");
                    SetGlobalVar(523, GetGlobalVar(523) + 1);
                    break;
                case 711:
                    Trace.Assert(originalScript == "npc.cast_spell( spell_reincarnation, picker_obj )");
                    npc.CastSpell(WellKnownSpells.Reincarnation, PickedObject);
                    break;
                case 730:
                    Trace.Assert(originalScript == "game.picker( npc, spell_cure_serious_wounds, should_heal_hp_on, [ 690, 640, 740 ] )");
                    // FIXME: picker;
                    break;
                case 740:
                    Trace.Assert(originalScript == "game.global_vars[524] = game.global_vars[524] + 1");
                ...
                case 980:
                case 981:
                    Trace.Assert(originalScript == "game.global_vars[455] |= 2**7");
                    SetGlobalVar(455, GetGlobalVar(455) | 0x80);
                    break;
                case 1000:
                    Trace.Assert(originalScript == "game.picker( npc, spell_reincarnation, should_clear_spell_on, [ 1045, 370, 1562 ] )");
                    // FIXME: picker;
                    break;
                case 1562:
                    Trace.Assert(originalScript == "game.global_vars[753] = picker_obj.stat_level_get(stat_experience); kill_picked(picker_obj, npc)");
                    SetGlobalVar(753, PickedObject.GetStat(Stat.experience));
                    kill_picked(PickedObject, npc);
                    ;
                    break;
                case 1575:
                    Trace.Assert(originalScript == "picker_obj.stat_base_set(stat_experience, game.global_vars[753]); npc.spells_pending_to_memorized()");
