    ...
    public class DickDialog : Dick, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 231:
                case 232:
                    Trace.Assert(originalScript == "game.quests[39].state == qs_mentioned and pc.skill_level_get(npc, skill_bluff) >= 10");
                    return GetQuestState(39) == QuestState.Mentioned && pc.GetSkillLevel(npc, SkillId.bluff) >= 10;
                case 233:
                case 234:
                    Trace.Assert(originalScript == "game.quests[39].state == qs_completed and pc.skill_level_get(npc, skill_bluff) >= 10");
