    ...
    public class HedrackDialog : Hedrack, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, out string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 55:
                case 56:
                    originalScript = "pc.skill_level_get(npc, skill_gather_information) >= 10 or pc.stat_level_get(stat_level_bard) >= 1";
                    return pc.GetSkillLevel(npc, SkillId.gather_information) >= 10 || pc.GetStat(Stat.level_bard) >= 1;
                case 57:
                case 472:
                    originalScript = "pc.stat_level_get(stat_level_cleric) >= 1 and pc.skill_level_get(npc, skill_diplomacy) >= 10";
                    return pc.GetStat(Stat.level_cleric) >= 1 && pc.GetSkillLevel(npc, SkillId.diplomacy) >= 10;
                case 58:
                case 473:
                    originalScript = "( game.party_alignment == CHAOTIC_EVIL or game.party_alignment == NEUTRAL_EVIL or game.party_alignment == LAWFUL_EVIL) and pc.skill_level_get(npc, skill_diplomacy) >= 10";
                ...
                case 141:
                case 142:
                    originalScript = "( pc.skill_level_get(npc, skill_gather_information) >= 10 or pc.stat_level_get(stat_level_bard) >= 1 ) and game.story_state <= 4";
        ...
        public void ApplySideEffect(GameObjectBody npc, GameObjectBody pc, int lineNumber, out string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 180:
                    ...
                    break;
                case 190:
                    originalScript = "game.global_flags[544] = 1";
                ...
                case 217:
                case 218:
                    originalScript = "game.quests[58].state = qs_completed; npc.item_transfer_to( pc, 3021 )";
                    SetQuestState(58, QuestState.Completed);
