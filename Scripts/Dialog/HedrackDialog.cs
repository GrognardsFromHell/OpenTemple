    ...
    public class HedrackDialog : Hedrack, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 57:
                case 472:
                    Trace.Assert(originalScript == "pc.stat_level_get(stat_level_cleric) >= 1 and pc.skill_level_get(npc, skill_diplomacy) >= 10");
                    return pc.GetStat(Stat.level_cleric) >= 1 && pc.GetSkillLevel(npc, SkillId.diplomacy) >= 10;
                case 58:
                case 473:
                    Trace.Assert(originalScript == "( game.party_alignment == CHAOTIC_EVIL or game.party_alignment == NEUTRAL_EVIL or game.party_alignment == LAWFUL_EVIL) and pc.skill_level_get(npc, skill_diplomacy) >= 10");
                    return (PartyAlignment == Alignment.CHAOTIC_EVIL || PartyAlignment == Alignment.NEUTRAL_EVIL || PartyAlignment == Alignment.LAWFUL_EVIL) && pc.GetSkillLevel(npc, SkillId.diplomacy) >= 10;
                case 141:
                case 142:
                    Trace.Assert(originalScript == "( pc.skill_level_get(npc, skill_gather_information) >= 10 or pc.stat_level_get(stat_level_bard) >= 1 ) and game.story_state <= 4");
                    return (pc.GetSkillLevel(npc, SkillId.gather_information) >= 10 || pc.GetStat(Stat.level_bard) >= 1) && StoryState <= 4;
                case 143:
                case 144:
        ...
        public void ApplySideEffect(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 180:
                    ...
                    break;
                case 190:
                    Trace.Assert(originalScript == "game.global_flags[544] = 1");
                ...
                case 217:
                case 218:
                    Trace.Assert(originalScript == "game.quests[58].state = qs_completed; npc.item_transfer_to( pc, 3021 )");
                    SetQuestState(58, QuestState.Completed);
                    npc.TransferItemByNameTo(pc, 3021);
                    ;
                    break;
                case 240:
                    Trace.Assert(originalScript == "game.map_flags( 5105, 1, 1 ); give_robes( npc, pc )");
                    // FIXME: map_flags;
                    give_robes(npc, pc);
                    ;
                    break;
