    ...
    public class ProsperousYoungerDDialog : ProsperousYoungerD, IDialogScript
    {
        public bool CheckPrecondition(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 405:
                    Trace.Assert(originalScript == "not pc.stat_level_get(stat_level_wizard) > 0");
                    return !(pc.GetStat(Stat.level_wizard) > 0);
                case 406:
                    Trace.Assert(originalScript == "pc.stat_level_get(stat_level_wizard) > 0");
                    return pc.GetStat(Stat.level_wizard) > 0;
                case 462:
                    Trace.Assert(originalScript == "game.party_alignment == CHAOTIC_EVIL or game.party_alignment == CHAOTIC_NEUTRAL or game.party_alignment == LAWFUL_EVIL or game.party_alignment == NEUTRAL_EVIL");
                ...
                case 501:
                case 502:
                    Trace.Assert(originalScript == "game.global_flags[867] == 0");
        ...
        public void ApplySideEffect(GameObjectBody npc, GameObjectBody pc, int lineNumber, string originalScript)
        {
            switch (lineNumber)
            {
                ...
                case 122:
                case 151:
                    Trace.Assert(originalScript == "pc.follower_add( npc ); buttin3(npc,pc,450)");
                    pc.AddFollower(npc);
                    buttin3(npc, pc, 450);
                    ;
                    break;
                ...
                case 351:
                    ...
                    break;
                case 482:
                    Trace.Assert(originalScript == "buttin4(npc,pc,400)");
                    buttin4(npc, pc, 400);
                    break;
                case 503:
                case 504:
                ...
                case 550:
                    ...
                    break;
                case 561:
                    Trace.Assert(originalScript == "game.global_flags[867] = 1; equip_transfer( npc, pc )");
                    SetGlobalFlag(867, true);
                    equip_transfer(npc, pc);
                    ;
                    break;
                case 22000:
                    Trace.Assert(originalScript == "game.global_vars[904] = 32");
                    SetGlobalVar(904, 32);
                    break;
                default:
                    Trace.Assert(originalScript == null);
