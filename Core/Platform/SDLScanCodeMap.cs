...
/**
 * Maps SDL Scan Codes to DirectInput8 keys.
 *
 * Taken under public domain from https://github.com/DanielGibson/Snippets/blob/master/sdl2_scancode_to_dinput.hv
 */
public static class SDLScanCodeMap
{
    public static DIK GetDIK(SDL.SDL_Scancode scancode)
    {
        var scancodeIdx = (int) scancode;
        if (scancodeIdx >= 0 && scancodeIdx < ScanCodeToKeyNum.Length)
        {
            return (DIK) ScanCodeToKeyNum[scancodeIdx];            
        }
    ...
    private static readonly int[] ScanCodeToKeyNum =
    {
