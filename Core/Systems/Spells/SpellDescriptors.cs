    ...
    public class SpellDescriptors : IEnumerable<SpellEntry>
    {
        ...
        public SpellDescriptors()
        {
            foreach (var path in Tig.FS.Search("rules/spells/*.txt"))
            {
                ...
                _spells[spellEnum] = ParseSpellFile(spellEnum, path);
            }
        }

        private static SpellEntry ParseSpellFile(int spellEnum, string path)
        {
            using var content = Tig.FS.OpenTextReader(path, Encoding.Default);
        ...
        private static readonly (string, UiPickerIncFlags)[] TargetIncludeFlags =
        {
            ...
            ("NonCritter", UiPickerIncFlags.UIPI_NonCritter),
        ...
        private static readonly (string, SpellDescriptor)[] Descriptors =
        {
            ...
            ("MIND_AFFECTING", SpellDescriptor.MIND_AFFECTING),
        ...
        private static bool ParseSpellLine(string line, out SpellLineKey key, out ReadOnlySpan<char> value)
        {
            ...
            {
                lowerCaseKey[i] = char.ToLowerInvariant(lowerCaseKey[i]);
            }
            ...
            else if (lowerCaseKeyReadOnly == "casting time")
            {
